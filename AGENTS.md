...
- **The two tiers never paint one object.** Inside an ending cell the chip is facet-filled and the ending it holds is plain ink (`.cell .end`) — two hues on one small object read as a gradient, not as two axes. The case is already stated by the section the cell sits in, so the ending need not restate it; the chip states the gender. An ending is never facet-coloured either.
