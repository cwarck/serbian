...
function renderNumberGroup(group) {
  ...
  const cells = group.rows.map(row => `
    <article class="num-mini">
      <span class="chart-label num-value">${row.n}</span>
      <span class="chart-form num-word" lang="sr">${SerbianFyi.sr(row.sr)}</span>
    </article>
  `).join('') + Array.from({ length: blanks }, () => '<article class="num-mini num-mini-empty" aria-hidden="true"></article>').join('');
  return `
    <section class="chart-group num-cardinals" data-tone="${group.tone}">
      ...
      <div class="chart-table">
        ...
        ${NUMBER_BUILDS.map(row => `
          <article class="chart-row">
            <span class="chart-cell chart-label num-value" data-label="${t('numbers.number')}">${row.n}</span>
            <span class="chart-cell chart-form num-built" data-label="${t('numbers.parts')}" lang="sr">${srParts(row.parts)}</span>
            <span class="chart-cell num-meaning" data-label="${t('numbers.meaning')}">${pick(row)}</span>
          </article>
        `).join('')}
      </div>
    </section>
